...
void runAuton() {
  ...
  if (autonType == RED_POSITIVE && scoreAllianceStake) {
    ...
    chassis.moveToPoint(chassis.getPose().x, 45, 1000, {.minSpeed = 100});
    chassis.waitUntilDone();
    pros::delay(1500);
    intake.brake();
    ...
    pros::delay(500);
    intake.brake();
    chassis.moveToPoint(-48, 38, 900);
    chassis.turnToHeading(0, 800);
    chassis.waitUntilDone();

    chassis.setPose(-50,40,0); // SET POSE HERE!!!
    ...
    intake.move(127);
    chassis.moveToPoint(-32, chassis.getPose().y, 1500, {.minSpeed = 100}); // Ring 1
    chassis.turnToHeading(135, 700, {.minSpeed = 100});

    chassis.moveToPose(27, -42, 90, 2500, {.minSpeed = 100}); // MOVE TO POSE
    chassis.waitUntilDone();
    ...
    chassis.turnToHeading(270, 700, {.minSpeed = 100});
    chassis.moveToPose(58, 10, 60, 3000, {.forwards = false, .lead = 0.8});
    clampSet(true);
  }
}